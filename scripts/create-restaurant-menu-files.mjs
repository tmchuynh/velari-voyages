/**
 * Restaurant Menu Generator Script
 * ================================
 *
 * This script generates TypeScript files containing detailed menu data for restaurants
 * associated with cities in the Velari Voyages project.
 * It creates or modifies files within a city-specific directory, typically named:
 * "src/lib/constants/cruises/restaurants/{cityName}/{restaurantName}Menu.ts" or
 * "src/lib/constants/cruises/restaurants/{cityName}/{restaurantName}-{menuType}Menu.ts"
 *
 * Each file exports an array variable named "{cityVar}{restaurantVar}Menu"
 * (e.g., londonTheSeafoodGrillMenu) containing one or more menu objects (main, dessert, drinks).
 *
 * Features:
 * - Generates diverse menu items for various categories (e.g., Starters, Main Courses, Desserts, Drinks).
 * - Customizes item names, prices, and dietary flags (vegetarian, vegan, gluten-free, etc.)
 *   based on restaurant cuisine, style, price range, and dietary focus.
 * - Supports generation of specific menu types (main, dessert, drinks) or all types.
 * - Allows targeting specific restaurants or all restaurants within a city.
 * - Provides options to customize the number of items per category.
 * - Supports appending new items to existing categories in a menu file.
 * - Offers advanced customization:
 *   - Exclude specific menu types or categories.
 *   - Set overall price range (budget, standard, premium, luxury).
 *   - Focus on specific dietary needs (vegan-only, vegetarian-only, gluten-free, etc.).
 *   - Define restaurant style (casual, fine-dining, adult-only, etc.).
 *   - Enhance specific menu sections (e.g., more drinks for adult-only venues).
 *
 * Usage Examples:
 * --------------
 * # Default: generates all menu types (main, dessert, drinks) for all restaurants
 * # in all cities, with default item counts and settings.
 * node scripts/create-restaurant-menu-files.mjs
 *
 * # Generate only main course menus for all restaurants:
 * node scripts/create-restaurant-menu-files.mjs --menu-type main
 *
 * # Generate menus with 10 items per category for all restaurants:
 * node scripts/create-restaurant-menu-files.mjs --items 10
 *
 * # Generate menus only for restaurants with "Bistro" in their name:
 * node scripts/create-restaurant-menu-files.mjs --restaurant "Bistro"
 *
 * # Focus generation on the "Signature Dishes" category for all restaurants:
 * # (Other categories will have a default smaller number of items)
 * node scripts/create-restaurant-menu-files.mjs --category "Signature Dishes"
 *
 * # Append 5 new items to the "Main Courses" category for "The Grand Bistro":
 * node scripts/create-restaurant-menu-files.mjs --append-category "Main Courses" --append-count 5 --restaurant "The Grand Bistro"
 *
 * # Exclude drinks menus and the "Soups" category for "Ocean View Grill":
 * node scripts/create-restaurant-menu-files.mjs --exclude-menu-types drinks --exclude-categories "Soups" --restaurant "Ocean View Grill"
 *
 * # Set a "luxury" price range for "The Diamond Club":
 * node scripts/create-restaurant-menu-files.mjs --price-range luxury --restaurant "The Diamond Club"
 *
 * # Create a vegan-only menu for "Green Garden Cafe":
 * node scripts/create-restaurant-menu-files.mjs --dietary-focus vegan-only --restaurant "Green Garden Cafe"
 *
 * # Set restaurant style to "fine-dining" for "Elegant Eats":
 * node scripts/create-restaurant-menu-files.mjs --restaurant-style fine-dining --restaurant "Elegant Eats"
 *
 * # Enhance the drinks menu for an "adult-only" restaurant named "Night Cap":
 * node scripts/create-restaurant-menu-files.mjs --restaurant-style adult-only --enhance-menu drinks --restaurant "Night Cap"
 *
 * Command-line Options:
 * ------------------
 * --menu-type <TYPE>        Specify menu type to generate.
 *                           Valid types: all, main, dessert, drinks. Default: all.
 *                           Example: --menu-type dessert
 *
 * --items <NUMBER>          Set the number of items to generate per category. Default: 7.
 *                           Example: --items 10
 *
 * --restaurant <NAME>       Filter restaurants by a partial name match (case-insensitive).
 *                           Default: all (processes all restaurants).
 *                           Example: --restaurant "Grill"
 *
 * --category <NAME>         Focus item generation on a specific category. Other categories
 *                           will receive a default smaller number of items. Default: all.
 *                           Example: --category "Chef's Specials"
 *
 * --append-category <NAME>  Enable Append Mode for the specified category.
 *                           Requires --restaurant to target a specific restaurant.
 *                           Example: --append-category "Appetizers"
 *
 * --append-count <NUMBER>   Number of items to append in Append Mode. Default: 3.
 *                           Example: --append-count 5
 *
 * --exclude-menu-types <TYPES> Comma-separated list of menu types to exclude.
 *                              Valid types: main, dessert, drinks.
 *                              Example: --exclude-menu-types drinks,dessert
 *
 * --exclude-categories <NAMES> Comma-separated list of category names to exclude.
 *                               Example: --exclude-categories "Soups,Salads"
 *
 * --price-range <RANGE>     Set the overall price range for generated menus.
 *                           Valid ranges: budget, standard, premium, luxury. Default: standard.
 *                           Example: --price-range premium
 *
 * --dietary-focus <FOCUS>   Set a specific dietary focus for menu generation.
 *                           Valid focuses: mixed, vegan-only, vegetarian-only, gluten-free,
 *                                          halal-only, kosher-only. Default: mixed.
 *                           Example: --dietary-focus vegan-only
 *
 * --restaurant-style <STYLE> Define the style of the restaurant, influencing menu content.
 *                            Valid styles: casual, fine-dining, family-friendly, adult-only.
 *                            Default: casual.
 *                            Example: --restaurant-style fine-dining
 *
 * --enhance-menu <TYPE>     Enhance a specific menu section (e.g., more drinks for adult-only).
 *                           Currently supports: drinks.
 *                           Example: --enhance-menu drinks
 *
 * Output Files:
 * ------------
 * The script generates TypeScript files in city-specific restaurant directories:
 * src/lib/constants/cruises/restaurants/{cityName}/{restaurantFilename}Menu.ts
 *
 * Each file exports a `RestaurantMenu[]` array, where each `RestaurantMenu` object
 * represents a type of menu (e.g., Main Course, Dessert, Drinks) and contains
 * categories with their respective items.
 * The `RestaurantMenu` type is expected to be imported from "@/lib/types/types".
 */
...
function generateCocktailItems(count, priceRange) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const description = getRandomItems(restaurantCocktailTemplates, 1)[0];
    ...
    if (mainMenuCategories.length > 0) {
      ...
      const getMainCourseDescription = (restaurant, cuisine) => {
        ...
        if (
          ...
          adjectives = fineDiningMenuAdjectives;
        } else if (
          args["restaurant-style"] === "adult-only" ||
          restaurant.isAdultOnly
        ) {
          adjectives = adultOnlyRestaurantAdjectives;
        } else if (args["restaurant-style"] === "family-friendly") {
          adjectives = familyFriendlyRestaurantAdjectives;
        } else {
          adjectives = casualDiningRestaurantAdjectives;
        }

        const adjective = getRandomItems(adjectives, 1)[0];
        let template = getRandomItems(restaurantMainCourseTemplates, 1)[0];
  ...
  if (
    ...
    if (dessertCategories.length > 0) {
      ...
      const getDessertMenuDescription = (restaurant, cuisine) => {
        ...
        const adjective = getRandomItems(adjectives, 1)[0];
        let template = getRandomItems(restaurantDessertMenuTemplates, 1)[0];
  ...
  if (
    ...
    if (drinkCategories.length > 0) {
      ...
      const getDrinkMenuDescription = (restaurant, cuisine) => {
        ...
        const adjective = getRandomItems(adjectives, 1)[0];
        let template = getRandomItems(restaurantDrinksMenuTemplates, 1)[0];
