import { Vessels } from "@/lib/interfaces/services/cruises";

export const aucklandVessels: Vessels[] = [
  {
    name: "MS Auckland Explorer",
    description:
      "The MS Auckland Explorer invites travelers to explore the treasures of Asia Pacific in unmatched comfort. This exceptional mid size cruise ship, based in Auckland, features curated excursions, immersive cultural programs, and luxurious accommodations that ensure an unforgettable journey.",
    type: "mid-size-cruise-ship",
    capacity: 1736,
    length: 447,
    width: 77,
    speed: 22,
    yearBuilt: 2022,
    homePort: {
      city: "Auckland",
      country: "New Zealand",
      coordinates: { latitude: -36.848461, longitude: 174.763336 },
    },
    specifications: {
      engineType: "Diesel-Electric Propulsion",
      fuelCapacity: 17200,
      waterCapacity: 520799,
      propulsionType: "Azipod Electric Propulsion",
      hullMaterial: "Reinforced Aluminum",
      classification: "ClassNK NS* (Special Purpose Ship)",
      safetyEquipment: [
        "Emergency escape breathing devices",
        "Searchlights and rescue equipment",
        "Radar reflectors",
        "Survival craft VHF radios",
        "Man overboard detection systems",
      ],
      navigationEquipment: [
        "Voyage data recorders (VDRs)",
        "Doppler speed logs",
        "Long-range identification and tracking (LRIT)",
      ],
      communicationEquipment: [
        "Ship-to-shore communication links",
        "Satellite communication systems",
        "Emergency position-indicating radio beacons",
      ],
      entertainmentEquipment: [
        "Multi-purpose event spaces",
        "Theater with Broadway-style productions",
      ],
      accessibilityFeatures: [
        "Elevator access to all public decks",
        "Transfer chairs for pool access",
        "Tactile guidance systems",
      ],
      environmentalFeatures: [
        "Dynamic routing to optimize fuel usage",
        "Noise reduction technology to minimize impact on marine life",
      ],
    },
    isLuxuryVessel: true,
    isPetFriendly: true,
  },
  {
    name: "SS Auckland Voyager",
    description:
      "The majestic SS Auckland Voyager represents the pinnacle of maritime engineering and hospitality. Based in Auckland, this exceptional mainstream cruise ship features elegant interiors, cutting-edge facilities, and carefully curated experiences that celebrate the unique culture of the Asia Pacific region.",
    type: "mainstream-cruise-ship",
    capacity: 2776,
    length: 655,
    width: 108,
    speed: 26,
    yearBuilt: 2018,
    homePort: {
      city: "Auckland",
      country: "New Zealand",
      coordinates: { latitude: -36.848461, longitude: 174.763336 },
    },
    specifications: {
      engineType: "Solar-Assisted Electric",
      fuelCapacity: 35370,
      waterCapacity: 832800,
      propulsionType: "Cycloidal Drive",
      hullMaterial: "Hybrid Steel-Composite",
      classification: "China Classification Society B",
      safetyEquipment: [
        "Radar reflectors",
        "Automated external defibrillators (AEDs)",
      ],
      navigationEquipment: [
        "Gyrocompasses and magnetic compasses",
        "Long-range identification and tracking (LRIT)",
      ],
      communicationEquipment: ["Maritime VHF radio systems"],
      entertainmentEquipment: [
        "Casino with table games and slot machines",
        "Cinema with latest releases",
        "Dance clubs and live music venues",
        "Library and educational facilities",
      ],
      accessibilityFeatures: [
        "Transfer chairs for pool access",
        "Height-adjustable furniture in select cabins",
        "Service animal accommodations",
      ],
      environmentalFeatures: [
        "Dynamic routing to optimize fuel usage",
        "Optimized hull design for reduced fuel consumption",
      ],
    },
    isLuxuryVessel: false,
    isPetFriendly: false,
  },
  {
    name: "SS Auckland Princess",
    description:
      "The SS Auckland Princess offers an unparalleled cruising experience from Auckland, combining luxury amenities with state-of-the-art navigation technology. This adults only ship features spacious cabins, gourmet dining options, and entertainment venues that showcase the best of Asia Pacific hospitality.",
    type: "adults-only-ship",
    capacity: 1580,
    length: 416,
    width: 72,
    speed: 22,
    yearBuilt: 2016,
    homePort: {
      city: "Auckland",
      country: "New Zealand",
      coordinates: { latitude: -36.848461, longitude: 174.763336 },
    },
    specifications: {
      engineType: "Azimuth Thrusters",
      fuelCapacity: 14970,
      waterCapacity: 474000,
      propulsionType: "Z-Drive Propulsion",
      hullMaterial: "Reinforced Aluminum",
      classification: "ClassNK NS* (Special Purpose Ship)",
      safetyEquipment: [
        "Advanced fire detection and suppression systems",
        "Survival craft VHF radios",
        "Emergency position indicating radio beacons (EPIRBs)",
      ],
      navigationEquipment: [
        "Electronic chart display information systems (ECDIS)",
        "Radar and automatic radar plotting aids (ARPAs)",
      ],
      communicationEquipment: [
        "Satellite communication systems",
        "Emergency position-indicating radio beacons",
      ],
      entertainmentEquipment: [
        "Theater with Broadway-style productions",
        "Virtual reality gaming center",
        "Casino with table games and slot machines",
        "Dance clubs and live music venues",
      ],
      accessibilityFeatures: [
        "Accessible entertainment venues",
        "Braille signage throughout",
        "Service animal accommodations",
        "Accessible dining facilities",
      ],
      environmentalFeatures: [
        "Biodegradable hydraulic fluids",
        "Advanced water treatment systems",
        "Exhaust gas cleaning systems",
      ],
    },
    isLuxuryVessel: false,
    isPetFriendly: false,
  },
  {
    name: "SY Auckland Legend",
    description:
      "Sailing from the picturesque port of Auckland, the SY Auckland Legend represents a new era in cruise experiences. This remarkable superyacht combines innovative design with thoughtful amenities, creating an atmosphere of refined elegance and warm hospitality.",
    type: "superyacht",
    capacity: 1030,
    length: 306,
    width: 55,
    speed: 17,
    yearBuilt: 2007,
    homePort: {
      city: "Auckland",
      country: "New Zealand",
      coordinates: { latitude: -36.848461, longitude: 174.763336 },
    },
    specifications: {
      engineType: "Diesel-Electric Propulsion",
      fuelCapacity: 8410,
      waterCapacity: 309000,
      propulsionType: "Counter-Rotating Propellers",
      hullMaterial: "Marine-Grade Steel",
      classification: "Bureau Veritas Star Class",
      safetyEquipment: [
        "Emergency position indicating radio beacons (EPIRBs)",
        "Emergency escape breathing devices",
        "Satellite emergency notification devices",
        "Digital safety monitoring systems",
        "Comprehensive medical facilities",
      ],
      navigationEquipment: [
        "Bridge alert management systems",
        "Radar and automatic radar plotting aids (ARPAs)",
        "Long-range identification and tracking (LRIT)",
      ],
      communicationEquipment: [
        "Internal crew communication network",
        "Broadband internet throughout",
      ],
      entertainmentEquipment: [
        "Casino with table games and slot machines",
        "Children's entertainment areas",
        "Multi-purpose event spaces",
        "Climbing walls and sports courts",
        "Dance clubs and live music venues",
      ],
      accessibilityFeatures: [
        "Assistive listening devices",
        "Braille signage throughout",
        "Accessible shore excursion options",
        "Accessible medical facilities",
      ],
      environmentalFeatures: [
        "Solar panel supplementation",
        "Energy-efficient appliances",
        "Ballast water management technology",
      ],
    },
    isLuxuryVessel: false,
    isPetFriendly: false,
  },
  {
    name: "TS Auckland Jewel",
    description:
      "The TS Auckland Jewel invites travelers to explore the treasures of Asia Pacific in unmatched comfort. This exceptional adults only ship, based in Auckland, features curated excursions, immersive cultural programs, and luxurious accommodations that ensure an unforgettable journey.",
    type: "adults-only-ship",
    capacity: 616,
    length: 223,
    width: 43,
    speed: 19,
    yearBuilt: 2020,
    homePort: {
      city: "Auckland",
      country: "New Zealand",
      coordinates: { latitude: -36.848461, longitude: 174.763336 },
    },
    specifications: {
      engineType: "Integrated Power Systems",
      fuelCapacity: 4790,
      waterCapacity: 184799,
      propulsionType: "Cycloidal Drive",
      hullMaterial: "Ice-Strengthened Steel",
      classification: "Bureau Veritas Star Class",
      safetyEquipment: [
        "Emergency position indicating radio beacons (EPIRBs)",
        "Marine evacuation systems (MES)",
        "Advanced fire detection and suppression systems",
        "Emergency escape breathing devices",
        "Survival craft VHF radios",
      ],
      navigationEquipment: [
        "Gyrocompasses and magnetic compasses",
        "Infrared camera systems",
      ],
      communicationEquipment: [
        "Emergency position-indicating radio beacons",
        "Marine radio telephony",
      ],
      entertainmentEquipment: [
        "Observation lounges with panoramic views",
        "Casino with table games and slot machines",
        "Multiple swimming pools and water features",
      ],
      accessibilityFeatures: [
        "Transfer chairs for pool access",
        "Hearing augmentation systems",
        "Wheelchair accessible cabins",
      ],
      environmentalFeatures: [
        "Heat recovery systems",
        "Advanced water treatment systems",
        "Exhaust gas cleaning systems",
      ],
    },
    isLuxuryVessel: false,
    isPetFriendly: true,
  },
];
