...
export async function fetchCruiseLinesFromVecto() {
  try {
    ...
    return response.data?.map((line: VectoCruiseLine) => ({
      id: line.id,
      name: line.name,
...
export async function fetchShipsFromVecto(cruiseLineId: string) {
  try {
    ...
    return response.data?.map((ship: VectoShip) => ({
      id: ship.id,
      name: ship.name,
 ...
 */
export async function fetchCruisesByDestinationFromVecto(destinationId: string) {
  try {
...
export async function fetchCruisesByDepartureCityFromVecto(
  ...
  try {
    ...
    console.error("Error fetching cruises by departure city from Vecto:", error);
    return [];
  }
}
